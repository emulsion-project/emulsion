...
Complex Multilayer::getAngle(const int &l1, const Complex &beta1,
		const double &alpha1, const int &l2) {
	kx = ki.t[l1] * cos(alpha1) * sin(beta1);
	ky = ki.t[l1] * sin(alpha1) * sin(beta1);
	if (beta1 <= piDblby2)
		return acos(calcKz(l2, true) / ki.t[l2]);
	else
		return acos(calcKz(l2, false) / ki.t[l2]);
}

void Multilayer::setSourceExt(const Complex &e0, const Complex &beta,
		const double &alpha) {
	a0 = Vector(2, 0.);
	alpha0 = alpha;
	beta0 = beta;
...
Vector3d Multilayer::sourceExtFields(const Complex &e0, const Complex &beta,
	...
	Vector a = exp(i_ * (kx * pos.t[0] + ky * pos.t[1]))
			* calcIncAmplIn(z, pol);
	int lay = getMedium(z);
	//cout << a0 << endl;
	kz = calcKz(lay, true);
...
Vector Multilayer::sourceExtAmpl(const Complex &e0, const Complex &beta,
		const double &alpha, const int &pol, const Vector3d &pos) {
	a0 = Vector(2, 0.);
	alpha0 = alpha;
	beta0 = beta;
...
Vector Multilayer::setSourceInt(const int &pol, const Complex &aP,
	...
	if (lay == 0) {
		...
		aS.t[1] = calcSMatrix(pol).t[0][0] * a.t[0]
				* exp(2. * i_ * kz * (Zi.t[0] - z));
		//aS.t[1] = calcSMatrix(pol).t[0][0]*a.t[0];//*exp(2.*i_*kz*(Zi.t[0]-z));
	} else if (lay == nbrMediums - 1) {
		aS.t[1] = 0.;
		kz = calcKz(nbrMediums - 1, true);
		aS.t[0] = calcSMatrix(pol).t[1][1] * a.t[1]
				* exp(2. * i_ * kz * (z - Zi.t[nbrMediums - 2]));
	} else
		aS = calcSourceAmplIn(z, pol);
	return aS;
}

Vector Multilayer::setSourceIntAmpl(const int &pol, const Complex &aP,
		const Complex &aM, const Complex &betaP, const double &alphaP,
	...
	if (layi == lay) {
		...
		kz = calcKz(0, true);
		aSi.t[1] = S.t[0][1] * (aS.t[1] + a.t[1])
				* exp(-i_ * kz * (z - Zi.t[0]));
		aSi.t[0] = 0.;
	} else if (layi == nbrMediums - 1) {
		S = calcSMatrixU(pol, zs);
		kz = calcKz(nbrMediums - 1, true);
		aSi.t[0] = S.t[1][0] * (aS.t[0] + a.t[0])
				* exp(i_ * kz * (z - Zi.t[nbrMediums - 2]));
		aSi.t[1] = 0.;
	} else if (z < zs) {
		aSi.t[0] = (aS.t[0] - S.t[1][1] * (aS.t[1] + a.t[1])) / S.t[1][0];
		aSi.t[1] = (S.t[0][0] * aS.t[0]
				+ (S.t[0][1] * S.t[1][0] - S.t[0][0] * S.t[1][1])
						* (aS.t[1] + a.t[1])) / S.t[1][0];
	} else if (z > zs) {
		aSi.t[0] = ((S.t[0][1] * S.t[1][0] - S.t[0][0] * S.t[1][1])
				* (aS.t[0] + a.t[0]) + S.t[1][1] * aS.t[1]) / S.t[0][1];
...
Vector3d Multilayer::setSourceIntFields(const int &pol, const Complex &aP,
		const Complex &aM, const Complex &betaP, const double &alphaP,
		const double &zs, const Vector3d &pos) {
	a = aS = Vector(2, 0.);
	Vector aSi(2, 0.);
...
Matrix Multilayer::calcSMatrixInt(const int &pol, const double &z1,
	...
	if (lay1 == 0)
		return calcSMatrixL(pol, z2);
	if (lay2 == nbrMediums - 1)
		return calcSMatrixU(pol, z1);
