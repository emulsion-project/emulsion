...
AxisSymParticle::AxisSymParticle() { T = R = NULL; }

void AxisSymParticle::setParticleProperties(const int &typ, const int &mat,const int &matExt, const double &lambd, const RVector &rr) {
  type = typ;
  material = mat;
  materialExt = matExt;
  lambda = lambd;
...
double AxisSymParticle::calcR(const double &cost) {
  double thet;
  if (type == 0) return geom.t[0];
  else if (type == 1) return geom.t[1]/sqrt(1.+(geom.t[1]*geom.t[1]/(geom.t[0]*geom.t[0])-1.)*cost*cost);
  else if (type == 2) {
    thet = acos(cost);
    if (thet < atan(geom.t[1]/geom.t[0])) return geom.t[0]/cost;
    else if (thet < piDbl-atan(geom.t[1]/geom.t[0])) return geom.t[1]/sin(thet);
    else return -geom.t[0]/cost;
  }
  return 0.;
}
...
double AxisSymParticle::calcDR(const double &cost) {
  ...
  if (type == 0) return 0.;
    ...
    if (geom.t[0] == geom.t[1]) return 0.;
    else return geom.t[1]*(geom.t[1]*geom.t[1]/(geom.t[0]*geom.t[0])-1.)*cost*sqrt(1.-cost*cost) / pow(1.+(geom.t[1]*geom.t[1]/(geom.t[0]*geom.t[0])-1.)*cost*cost,1.5);
  }
  else if (type == 2) {
    thet = acos(cost);
    if (thet < atan(geom.t[1]/geom.t[0])) return geom.t[0]*sin(thet)/(cost*cost);
    else if (thet < piDbl-atan(geom.t[1]/geom.t[0])) return -geom.t[0]*cost/(sin(thet)*sin(thet));
    else return -geom.t[0]*sin(thet)/(cost*cost);
  }
  return 0.;
}

void AxisSymParticle::setTMatrixParameters(const int &n, const int &ng, const int &meth) {
  N = n;
  Ng = ng;
  method = meth;
  if (T != NULL) delete [] T;
  ...
  if (type == 0) calcTSphere();
  else {
    if (method == 0) {
      if (type == 2) calcTLScylinder();
      else calcTEBCM();
    }
    if (method == 1) calcTBC();
    if (method == 2) calcTLS();
...
void AxisSymParticle::calcTSphere() {
  ...
  for (int m=-N ; m<=N ; m++) {
    ...
    for (int n=abs(m) ; n<=N ; n++) {
      if (n==0) n++;
      T[i].t[l+nbr][l+nbr] = -(m2*m2*bes2.jn(n)*bes1.djn(n)-bes2.djn(n)*bes1.jn(n))/(m2*m2*bes2.jn(n)*bes1.dh1n(n)-bes2.djn(n)*bes1.h1n(n)); // -an
      T[i].t[l][l] = -(bes1.jn(n)*bes2.djn(n)-bes1.djn(n)*bes2.jn(n))/(bes2.djn(n)*bes1.h1n(n)-bes2.jn(n)*bes1.dh1n(n)); // -bn
      ...
      R[i].t[l][l] = -i_*m2/(k2*geom.t[0]*bes2.jn(n)*bes1.djn(n)-m2*bes2.djn(n)*k1*geom.t[0]*bes1.jn(n)); // -cn
      R[i].t[l+nbr][l+nbr] = -i_*m2/(m2*k2*geom.t[0]*bes2.jn(n)*bes1.djn(n)-bes2.djn(n)*k1*geom.t[0]*bes1.jn(n)); // -dn
      l++;
    }
    i++;
  }
    ...
    for (int m=-N ; m<=N ; m++) {
      ...
      for (int nl=abs(m) ; nl<=N ; nl++) {
        ...
        for (int nc=abs(m) ; nc<=N ; nc++) {
          ...
          Q11[m+N].t[l][c] += w*(dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS)+ n2/n1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS));
          Q11[m+N].t[l][c+nbr] += w*(dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS) + n2/n1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS));
          Q11[m+N].t[l+nbr][c] += w*(dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS) + n2/n1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS));
          Q11[m+N].t[l+nbr][c+nbr] += w*(dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS) + n2/n1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS));

          vswf1.calcVSWFs(-m,nl,3,false);
          Q31[m+N].t[l][c] -= w*(dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS)+ n2/n1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS));
          Q31[m+N].t[l][c+nbr] -= w*(dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS) + n2/n1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS));
          Q31[m+N].t[l+nbr][c] -= w*(dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS) + n2/n1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS));
          Q31[m+N].t[l+nbr][c+nbr] -= w*(dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS) + n2/n1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS));
          c++;
        }
        l++;
      }
    ...
    for (int m=-N ; m<=N ; m++) {
      ...
      for (int nl=abs(m) ; nl<=N ; nl++) {
        ...
        for (int nc=abs(m) ; nc<=N ; nc++) {
          ...
          A[m+N].t[l+2*nbr][c] += w*k1*dotProduct(crossProduct(vswf1.Nmn,vswf2.Mmn),ndS);
          A[m+N].t[l+2*nbr][c+nbr] += w*k1*dotProduct(crossProduct(vswf1.Mmn,vswf2.Mmn),ndS);
          A[m+N].t[l+3*nbr][c] += w*k1*dotProduct(crossProduct(vswf1.Nmn,vswf2.Nmn),ndS);
          A[m+N].t[l+3*nbr][c+nbr] += w*k1*dotProduct(crossProduct(vswf1.Mmn,vswf2.Nmn),ndS);
          ...
          A[m+N].t[l+2*nbr][c+2*nbr] -= w*k2*dotProduct(crossProduct(vswf2c.Nmn,vswf2.Mmn),ndS);
          A[m+N].t[l+2*nbr][c+3*nbr] -= w*k2*dotProduct(crossProduct(vswf2c.Mmn,vswf2.Mmn),ndS);
          A[m+N].t[l+3*nbr][c+2*nbr] -= w*k2*dotProduct(crossProduct(vswf2c.Nmn,vswf2.Nmn),ndS);
          A[m+N].t[l+3*nbr][c+3*nbr] -= w*k2*dotProduct(crossProduct(vswf2c.Mmn,vswf2.Nmn),ndS);
          ...
          B[m+N].t[l+2*nbr][c] -= w*k1*dotProduct(crossProduct(vswf1.Nmn,vswf2.Mmn),ndS);
          B[m+N].t[l+2*nbr][c+nbr] -= w*k1*dotProduct(crossProduct(vswf1.Mmn,vswf2.Mmn),ndS);
          B[m+N].t[l+3*nbr][c] -= w*k1*dotProduct(crossProduct(vswf1.Nmn,vswf2.Nmn),ndS);
          B[m+N].t[l+3*nbr][c+nbr] -= w*k1*dotProduct(crossProduct(vswf1.Mmn,vswf2.Nmn),ndS);
          c++;
        }
        l++;
      }
    ...
    for (int m=-N ; m<=N ; m++) {
      ...
      for (int nl=abs(m) ; nl<=N ; nl++) {
        ...
        for (int nc=abs(m) ; nc<=N ; nc++) {
          ...
          vswf2.calcVSWFs(m,nl,1,false);
          vswf2c.calcVSWFs(m,nc,1,false);
          A[m+N].t[l][c] += w*(dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Mmn)) + dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Nmn)));
          A[m+N].t[l][c+nbr] += w*(dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Mmn)) + dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Nmn)));
          A[m+N].t[l+nbr][c] += w*(dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Nmn)) + dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Mmn)));
          A[m+N].t[l+nbr][c+nbr] += w*(dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Nmn)) + dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Mmn)));
          ...
          A[m+N].t[l][c+2*nbr] -= w*(dotProduct(crossProduct(ndS,vswf2c.Mmn),crossProduct(ndS,vswf1.Mmn)) + k2/k1*dotProduct(crossProduct(ndS,vswf2c.Nmn),crossProduct(ndS,vswf1.Nmn)));
          A[m+N].t[l][c+3*nbr] -= w*(dotProduct(crossProduct(ndS,vswf2c.Nmn),crossProduct(ndS,vswf1.Mmn)) + k2/k1*dotProduct(crossProduct(ndS,vswf2c.Mmn),crossProduct(ndS,vswf1.Nmn)));
          A[m+N].t[l+nbr][c+2*nbr] -= w*(dotProduct(crossProduct(ndS,vswf2c.Mmn),crossProduct(ndS,vswf1.Nmn)) + k2/k1*dotProduct(crossProduct(ndS,vswf2c.Nmn),crossProduct(ndS,vswf1.Mmn)));
          A[m+N].t[l+nbr][c+3*nbr] -= w*(dotProduct(crossProduct(ndS,vswf2c.Nmn),crossProduct(ndS,vswf1.Nmn)) + k2/k1*dotProduct(crossProduct(ndS,vswf2c.Mmn),crossProduct(ndS,vswf1.Mmn)));
          ...
          B[m+N].t[l][c] -= w*(dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Mmn)) + dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Nmn)));
          B[m+N].t[l][c+nbr] -= w*(dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Mmn)) + dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Nmn)));
          B[m+N].t[l+nbr][c] -= w*(dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Nmn)) + dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Mmn)));
          B[m+N].t[l+nbr][c+nbr] -= w*(dotProduct(crossProduct(ndS,vswf1c.Nmn),crossProduct(ndS,vswf1.Nmn)) + dotProduct(crossProduct(ndS,vswf1c.Mmn),crossProduct(ndS,vswf1.Mmn)));
...
void AxisSymParticle::calcTLScylinder() {
  ...
  for (int interv=0 ; interv<3 ; interv++) {
    if (interv == 0) gaussLeg = gaussLegQuad(Ng,-1.,cos(piDbl-atan(geom.t[1]/geom.t[0])));
    if (interv == 1) gaussLeg = gaussLegQuad(Ng,cos(piDbl-atan(geom.t[1]/geom.t[0])),cos(atan(geom.t[1]/geom.t[0])));
    if (interv == 2) gaussLeg = gaussLegQuad(Ng,cos(atan(geom.t[1]/geom.t[0])),1.);
    for (int k=0 ; k<Ng ; k++) {
      cost = gaussLeg.t[k][0];
      w = gaussLeg.t[k][1];
      r = calcR(cost);
      ...
      for (int m=-N ; m<=N ; m++) {
        ...
        for (int nl=abs(m) ; nl<=N ; nl++) {
          ...
          for (int nc=abs(m) ; nc<=N ; nc++) {
            ...
            vswf1.calcVSWFs(-m,nl,1,false);
            vswf2.calcVSWFs(m,nc,1,false);
            Q11[m+N].t[l][c] += w*(k1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS)+ k2*dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS));
            Q11[m+N].t[l][c+nbr] += w*(k1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS) + k2*dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS));
            Q11[m+N].t[l+nbr][c] += w*(k1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS) + k2*dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS));
            Q11[m+N].t[l+nbr][c+nbr] += w*(k1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS) + k2*dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS));

            vswf1.calcVSWFs(-m,nl,3,false);
            Q31[m+N].t[l][c] -= w*(k1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS)+ k2*dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS));
            Q31[m+N].t[l][c+nbr] -= w*(k1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS) + k2*dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS));
            Q31[m+N].t[l+nbr][c] -= w*(k1*dotProduct(crossProduct(vswf2.Mmn,vswf1.Mmn),ndS) + k2*dotProduct(crossProduct(vswf2.Nmn,vswf1.Nmn),ndS));
            Q31[m+N].t[l+nbr][c+nbr] -= w*(k1*dotProduct(crossProduct(vswf2.Nmn,vswf1.Mmn),ndS) + k2*dotProduct(crossProduct(vswf2.Mmn,vswf1.Nmn),ndS));
            c++;
          }
          l++;
        }
...
void AxisSymParticle::calcSources() {
  ...
  if (geom.t[0] >= geom.t[1]) {
    ...
    for (int i=0 ; i<N ; i++) sources.t[i] = (-d + double(i)*step);
  }
  else {
    double d = 2.*geom.t[1];
    double step = d/double(N+1);
    for (int i=0 ; i<N ; i++) sources.t[i] = i_*(-d*0.5 + double(i+1)*step);
  }
  return;
}
    ...
    for (int m=-N ; m<=N ; m++) {
      ...
      for (int nl=abs(m) ; nl<=N ; nl++) {
        ...
        for (int nc=1 ; nc<=N ; nc++) {
          ...
          Q11[m+N].t[l][c] += mult*(k1*dotProduct(crossProduct(vswf2ds.Mmn,vswf1.Nmn),ndS)+ k2*dotProduct(crossProduct(vswf2ds.Nmn,vswf1.Mmn),ndS));
          Q11[m+N].t[l][c+N] += mult*(k1*dotProduct(crossProduct(vswf2ds.Nmn,vswf1.Nmn),ndS) + k2*dotProduct(crossProduct(vswf2ds.Mmn,vswf1.Mmn),ndS));
          Q11[m+N].t[l+nbr][c] += mult*(k1*dotProduct(crossProduct(vswf2ds.Mmn,vswf1.Mmn),ndS) + k2*dotProduct(crossProduct(vswf2ds.Nmn,vswf1.Nmn),ndS));
          Q11[m+N].t[l+nbr][c+N] += mult*(k1*dotProduct(crossProduct(vswf2ds.Nmn,vswf1.Mmn),ndS) + k2*dotProduct(crossProduct(vswf2ds.Mmn,vswf1.Nmn),ndS));
          c++;
        }
        l++;
      }
    ...
    for (int m=-N ; m<=N ; m++) {
      ...
      for (int nl=1 ; nl<=N ; nl++) {
        ...
        for (int nc=abs(m) ; nc<=N ; nc++) {
          ...
          Q311[m+N].t[l][c] += w*(dotProduct(crossProduct(vswf1.Mmn,vswf1ds.Nmn),ndS)+ dotProduct(crossProduct(vswf1.Nmn,vswf1ds.Mmn),ndS));
          Q311[m+N].t[l][c+nbr] += w*(dotProduct(crossProduct(vswf1.Nmn,vswf1ds.Nmn),ndS) + dotProduct(crossProduct(vswf1.Mmn,vswf1ds.Mmn),ndS));
          Q311[m+N].t[l+N][c] += w*(dotProduct(crossProduct(vswf1.Mmn,vswf1ds.Mmn),ndS) + dotProduct(crossProduct(vswf1.Nmn,vswf1ds.Nmn),ndS));
          Q311[m+N].t[l+N][c+nbr] += w*(dotProduct(crossProduct(vswf1.Nmn,vswf1ds.Mmn),ndS) + dotProduct(crossProduct(vswf1.Mmn,vswf1ds.Nmn),ndS));
          c++;
          }
        l++;
      }
    ...
    for (int m=-N ; m<=N ; m++) {
      ...
      for (int nl=1 ; nl<=N ; nl++) {
        ...
        for (int nc=1 ; nc<=N ; nc++) {
          vswf2ds.calcVSWFsds(m,nc,1,false);
          vswf1ds.calcVSWFsds(-m,nl,3,false);
          Q312[m+N].t[l][c] += w*(dotProduct(crossProduct(vswf2ds.Mmn,vswf1ds.Nmn),ndS)+ rap*dotProduct(crossProduct(vswf2ds.Nmn,vswf1ds.Mmn),ndS));
          Q312[m+N].t[l][c+N] += w*(dotProduct(crossProduct(vswf2ds.Nmn,vswf1ds.Nmn),ndS) + rap*dotProduct(crossProduct(vswf2ds.Mmn,vswf1ds.Mmn),ndS));
          Q312[m+N].t[l+N][c] += w*(dotProduct(crossProduct(vswf2ds.Mmn,vswf1ds.Mmn),ndS) + rap*dotProduct(crossProduct(vswf2ds.Nmn,vswf1ds.Nmn),ndS));
          Q312[m+N].t[l+N][c+N] += w*(dotProduct(crossProduct(vswf2ds.Nmn,vswf1ds.Mmn),ndS) + rap*dotProduct(crossProduct(vswf2ds.Mmn,vswf1ds.Nmn),ndS));
          c++;
        }
        l++;
      }
  ...
  for (int m=-N ; m<=N ; m++) {
    ...
    for (int nl=abs(m) ; nl<=N ; nl++) {
      ...
      for (int nc=abs(m) ; nc<=N ; nc++) {
        ...
        TT.t[ii(nl,m)+NM][ii(nc,m)+NM] = T[m+N].t[l+nbr][c+nbr];
        c++;
      }
      l++;
    }
  ...
  for (int m=-N ; m<=N ; m++) {
    ...
    for (int nl=abs(m) ; nl<=N ; nl++) {
      ...
      for (int nc=abs(m) ; nc<=N ; nc++) {
        ...
        RR.t[ii(nl,m)+NM][ii(nc,m)+NM] = R[m+N].t[l+nbr][c+nbr];
        c++;
      }
      l++;
    }
...
Vector AxisSymParticle::multT(const Vector &v) {
  ...
  for (int m=-N ; m<=N ; m++) {
    ...
    for (int nl=abs(m) ; nl<=N ; nl++) {
      ...
      for (int nc=abs(m) ; nc<=N ; nc++) {
        if (nc == 0) nc++;
        vv.t[ii(nl,m)] += T[m+N].t[l][c]*v.t[ii(nc,m)] + T[m+N].t[l][c+nbr]*v.t[ii(nc,m)+NM];
        vv.t[ii(nl,m)+NM] += T[m+N].t[l+nbr][c]*v.t[ii(nc,m)] + T[m+N].t[l+nbr][c+nbr]*v.t[ii(nc,m)+NM];
        c++;
      }
      l++;
    }
