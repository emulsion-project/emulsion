...
Multilayer1::Multilayer1() {
}

Multilayer1::Multilayer1(const string &fname, const double &lambd) {
	setParameters(fname, lambd);
}

void Multilayer1::setParameters(const string &fname, const double &lambd) {
	string str, fileName;
...
void Multilayer1::setLambda(const double &lambd) {
	lambda = lambd;
...
void Multilayer1::setKxy(const Complex &k) {
	kxy = k;
	return;
}

int Multilayer1::getMedium(const double &z) {
	if (nbrMediums == 1)
		return 0;
	for (int i = 0; i < nbrMediums - 1; i++)
		if (z <= Zi.t[i])
			return i;
	return nbrMediums - 1;
}

Complex Multilayer1::calcKz(const int &lay, const bool &dirP) {
	Complex K = sqrt(ki.t[lay] * ki.t[lay] - kxy * kxy);
	if (!dirP)
		K *= -1.;
	if ((dirP && K.im < 0.) || (!dirP && K.im > 0.))
		K *= -1.;
	return K;
}

Vector Multilayer1::sourceExtOut(const Vector &a, const int &pol) {
	Vector aa = calcSMatrix(pol) * a;
	//swap(aa.t[0],aa.t[1]);
	return aa;
}

Vector Multilayer1::sourceExtIn(const Vector &a, const int &pol,
		const double &z) {
	Vector aPM(2, 0.), aa;
	int lay = getMedium(z);
	if (lay == 0) {
...
Vector Multilayer1::sourceInt(const int &pol, const Vector &a, const double &zs,
		const double &z) { // a = aP & aM
	aS = Vector(2, 0.);
	Vector aSi(2, 0.);
	Vector aa(2, 0.);
	...
	if (lay == 0) {
		aS.t[0] = 0.;
		kz = calcKz(0, true);
		aS.t[1] = calcSMatrix(pol).t[0][0] * a.t[0]
				* exp(2. * i_ * kz * (Zi.t[0] - zs));
	} else if (lay == nbrMediums - 1) {
		aS.t[1] = 0.;
		kz = calcKz(nbrMediums - 1, true);
		aS.t[0] = calcSMatrix(pol).t[1][1] * a.t[1]
				* exp(2. * i_ * kz * (zs - Zi.t[nbrMediums - 2]));
	} else {
...
Vector Multilayer1::calcFresnelCoefsTE(const int &i, const int &j) {
	Vector coefs(2, 0.);
	if (i < j) {
		Kzi = calcKz(i, true);
		Kzj = calcKz(j, true);
...
Vector Multilayer1::calcFresnelCoefsTM(const int &i, const int &j) {
	Complex Kzi, Kzj;
	Vector coefs(2, 0.);
	if (i < j) {
		Kzi = calcKz(i, true);
	...
	coefs.t[0] = -(ni.t[i] * ni.t[i] * Kzj - ni.t[j] * ni.t[j] * Kzi)
			/ (ni.t[j] * ni.t[j] * Kzi + ni.t[i] * ni.t[i] * Kzj); //rTM, rb, rp
	coefs.t[1] = (2. * ni.t[i] * ni.t[j] * Kzi)
			/ (ni.t[j] * ni.t[j] * Kzi + ni.t[i] * ni.t[i] * Kzj); // tTM
	return coefs;
}

Matrix Multilayer1::calcSl(const int &l, const int &u, const int &pol) {
	Matrix S(2, 2);
...
Matrix Multilayer1::calcSh(const int &i, const double &h) {
	Matrix S(2, 2);
	kz = calcKz(i, true);
	S.t[0][0] = S.t[1][1] = 0.;
...
Matrix Multilayer1::calcSMatrix(const int &pol) { // 0:TE 1:TM
	Matrix S, Su, Sl;
	Complex cst;
...
Matrix Multilayer1::calcSMatrixU(const int &pol, const double &z) { // 0:TE 1:TM
	Matrix S, Su, Sl;
	S = Su = Sl = Matrix(2, 2, 0.);
	int lay = getMedium(z);
	if (lay == 0) {
		...
		Su = calcSMatrix(pol);
		S.t[0][0] = Sl.t[0][0]
				+ Sl.t[0][1] * Su.t[0][0] / (1. - Su.t[0][0] * Sl.t[1][1])
						* Sl.t[1][0];
		S.t[0][1] = Sl.t[0][1] / (1. - Su.t[0][0] * Sl.t[1][1]) * Su.t[0][1];
		S.t[1][0] = Sl.t[1][0] / (1. - Su.t[0][0] * Sl.t[1][1]) * Su.t[1][0];
		S.t[1][1] = Su.t[1][1]
				+ Su.t[1][0] * Sl.t[1][1] / (1. - Su.t[0][0] * Sl.t[1][1])
						* Su.t[0][1];
		return S;
	}
	//if (lay == nbrMediums-1) return S;
	if (lay == nbrMediums - 1)
		return calcSh(nbrMediums - 1, fabs(z - Zi.t[nbrMediums - 2]));

	Su = calcSl(nbrMediums - 2, nbrMediums - 1, pol);

	for (int i = nbrMediums - 2; i > 0; i--) {
		if (lay == i)
			Sl = calcSh(i, fabs(Zi.t[i] - z));
		else
			Sl = calcSh(i, fabs(Zi.t[i - 1] - Zi.t[i]));
...
Matrix Multilayer1::calcSMatrixL(const int &pol, const double &z) { // 0:TE 1:TM
	Matrix S, Su, Sl;
	S = Su = Sl = Matrix(2, 2, 0.);
	int lay = getMedium(z);
	if (lay == nbrMediums - 1) {
	...
	//if (lay == 0) return S;
	if (lay == 0)
		return calcSh(0, fabs(z - Zi.t[0]));
	Sl = calcSl(0, 1, pol);

	for (int i = 1; i < nbrMediums; i++) {
		if (lay == i)
			Su = calcSh(i, fabs(z - Zi.t[i - 1]));
		else
			Su = calcSh(i, fabs(Zi.t[i] - Zi.t[i - 1]));
...
Matrix Multilayer1::calcSMatrixInt(const int &pol, const double &z1,
		const double &z2) { // 0:TE 1:TM
	Matrix S, Su, Sl;
	S = Su = Sl = Matrix(2, 2, 0.);
	int lay1 = getMedium(z1);
	int lay2 = getMedium(z2);
	if (lay1 == lay2)
		return calcSh(lay1, fabs(z2 - z1));
	if (lay1 == 0) {
