...
Complex operator/(const Complex &z1, const Complex &z2) {
	...
	return Complex((z1.re * z2.re + z1.im * z2.im) / (m),
			(z1.im * z2.re - z1.re * z2.im) / (m));
}
...
bool operator<(const Complex &z1, const Complex &z2) {
	double abs1 = sqrt(z1.re * z1.re + z1.im * z1.im), abs2 = sqrt(
...
Complex sqrt(const Complex &z) {
	return polar(sqrt(abs(z)), phase(z) / 2.);
}
Complex exp(const Complex &z) {
	return cart(exp(z.re) * std::cos(z.im), exp(z.re) * std::sin(z.im));
}
Complex log(const Complex &z) {
	return cart(log(abs(z)), phase(z));
}
Complex log10(const Complex &z) {
	return log(z) / log(10.);
}
Complex pow(const Complex &z, const double &x) {
	return polar(pow(abs(z), x), phase(z) * x);
}
...
Complex powi(const int &n) {
	if ((n % 2) == 0)
		return Complex(real(pow(i_, n)), 0.);
...
Complex sin(const Complex &z) {
	return cart(sin(z.re) * cosh(z.im), cos(z.re) * sinh(z.im));
}
Complex cos(const Complex &z) {
	return cart(cos(z.re) * cosh(z.im), -sin(z.re) * sinh(z.im));
}
...
Complex asin(const Complex &z) {
	return -i_ * log(i_ * z + sqrt(1. - z * z));
}
Complex acos(const Complex &z) {
	return -i_ * log(z + sqrt(z * z - 1.));
}
Complex atan(const Complex &z) {
	return i_ * log((1. - i_ * z) / (1. + i_ * z)) / 2.;
}
Complex cosh(const Complex &z) {
	return cart(cos(z.im) * cosh(z.re), sin(z.im) * sinh(z.re));
}
Complex sinh(const Complex &z) {
	return cart(sinh(z.re) * cos(z.im), cosh(z.re) * sin(z.im));
}
