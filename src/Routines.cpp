...
void calcCrossSections(const string &fname, const double &lambda1,
	...
	for (double ll = lambda1; ll <= lambda2 * 1.0001; ll += step) {
		...
		for (int i = 0; i < Cabsi.n; i++)
			Cabs += Cabsi.t[i];
		cout << ll * 1e9 << "\t" << Cabs << "\t" << Cext - Cabs << "\t" << Cext
				<< endl;
		of << 2. * piDbl * c * hbar / (e * ll) << "\t" << Cabs << "\t"
				<< Cext - Cabs << "\t" << Cext << endl;
	}
}
...
void calcEnergyLossProbability(const string &fname, const double &e1,
	...
	for (double ee = e1; ee <= e2 * 1.0001; ee += step) {
		...
		cout << ee << "\t" << ms.lambda * 1e9 << "\t" << proba << "\t" << CL
				<< endl;
		of << ee << "\t" << ms.lambda * 1e9 << "\t" << (proba) << "\t" << CL
				<< endl;
	}
}

void calcNearFieldPlane(const string &fname, const int &N1, const int &N2,
	...
	RMatrix E1r(N1, N2), E1i(N1, N2), E2r(N1, N2), E2i(N1, N2), E3r(N1, N2),
			E3i(N1, N2);
	...
	for (int t = 0; t < N1; t++) {
		...
		for (int p = 0; p < N2; p++) {
			...
			if (!cart)
				field = toSphericalCoordinates(field, T, P);
			X.t[t][p] = xx;
			Y.t[t][p] = yy;
			Z.t[t][p] = zz;
...
void calcNearFieldParticleSurface(const string &fname, const int &N1,
		const int &N2, const bool &cart) {
	MultipleScattering ms(fname);
	cout << "Solving the linear system...";
	ms.setLambda(720e-9);
	ms.solve();
	cout << "done" << endl;
	if (ms.incidence.type == 1)
		ms.incidence.setElectron(ms.N, ms.incidence.E0, ms.incidence.E, 0., 0.);
	RMatrix X(N1, N2 * ms.Np), Y(N1, N2 * ms.Np), Z(N1, N2 * ms.Np);
	RMatrix E1r(N1, N2 * ms.Np), E1i(N1, N2 * ms.Np), E2r(N1, N2 * ms.Np), E2i(
			N1, N2 * ms.Np), E3r(N1, N2 * ms.Np), E3i(N1, N2 * ms.Np);
	ofstream of1("near_field/nbrPlots.txt");
	of1 << ms.Np << "\t" << N1 << "\t" << N2;
...
void calcEELSMapping(const string &fname, const int &N1, const int &N2,
		const double &t1, const double &t2, const double &p1,
		const double &p2) {
	MultipleScattering ms(fname);
	double XX, YY;
	RMatrix X(N1, N2), Y(N1, N2), Z(N1, N2);
	...
	for (int t = 0; t < N1; t++) {
		...
		for (int p = 0; p < N2; p++) {
			...
			for (int i = 0; i < ms.Np; i++) {
				...
				YY = yy - ms.y[i];
				R = sqrt(XX * XX + YY * YY);
				if (R < (ms.particles[i].geom.t[0]) * 0.9999999)
					calc = false;
			}
			if (calc) {
