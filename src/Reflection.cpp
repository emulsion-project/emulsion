...
void Reflection::setConstants(const double &lambd, const int &n,
		const int &ng) {
	if ((N != n) || (Ng != ng) || (lambd != lambda)) {
		...
		xMin = 0.;
		for (int i = 0; i < nbrLay; i++)
			if (xMin < real(ml.ki.t[i]))
				xMin = real(ml.ki.t[i]);
		xMin += 2. * piDbl / lambd;
		xMax = xMin * 200.;
		yMax = xMin / 10.;
		...
		for (int l = 0; l < nbrLay; l++) {
			...
			for (int i = 0; i < Ng; i++) {
				...
				kappa.t[i] = xMin * 0.5 * (1. - cos(gaussLeg1.t[i][0]))
						- i_ * yMax * sin(gaussLeg1.t[i][0]);
				dkappa.t[i] = xMin * 0.5 * sin(gaussLeg1.t[i][0])
						- i_ * yMax * cos(gaussLeg1.t[i][0]);
				...
				for (int nn = 1; nn <= N; nn++) {
					...
					for (int m = -nn; m <= nn; m++) {
						legPi.t[i + 2 * l * Ng][ii(nn, m)] = cstn * double(m)
								* leg.Pimn(nn, m);
						legTau.t[i + 2 * l * Ng][ii(nn, m)] = cstn
								* leg.Taumn(nn, m);
					}
				}
				...
				for (int nn = 1; nn <= N; nn++) {
					cstn = 1. / sqrt(double(2 * nn * (nn + 1)));
					for (int m = -nn; m <= nn; m++) {
						legPi.t[i + Ng + 2 * l * Ng][ii(nn, m)] = cstn
								* double(m) * leg.Pimn(nn, m);
						legTau.t[i + Ng + 2 * l * Ng][ii(nn, m)] = cstn
								* leg.Taumn(nn, m);
					}
				}
			}
...
void Reflection::setParameters(const double &xx, const double &yy,
		const double &zzi, const double &zzj, const bool &self) {
...
Matrix Reflection::calcRSelf() {
	...
	for (int k = 0; k < 2 * Ng; k++) {
		...
		ml.setKxy(kappa.t[k]);
		cstMult = gaussLeg.t[k][1] * 4. * kappa.t[k] * dkappa.t[k]
				/ ml.ki.t[lay] / kz.t[k + 2 * lay * Ng];
		line = 0;
		for (int n = 1; n <= N; n++) {
			for (int m = -n; m <= n; m++) {
				a.t[1] = powm1(n - m);
				...
				for (int l = 1; l <= N; l++) {
					for (int kk = -l; kk <= l; kk++) {
						if (m == kk) {
							...
							PP = legPi.t[k + 2 * lay * Ng][ii(n, m)]
									* legPi.t[k + 2 * lay * Ng][ii(l, m)];
							TT = legTau.t[k + 2 * lay * Ng][ii(n, m)]
									* legTau.t[k + 2 * lay * Ng][ii(l, m)];
							PT = legPi.t[k + 2 * lay * Ng][ii(n, m)]
									* legTau.t[k + 2 * lay * Ng][ii(l, m)];
							TP = legTau.t[k + 2 * lay * Ng][ii(n, m)]
									* legPi.t[k + 2 * lay * Ng][ii(l, m)];
	...
	for (int k = 0; k < 2 * Ng; k++) {
		...
		for (int n = 1; n <= N; n++) {
			for (int m = -n; m <= n; m++) {
				...
				for (int l = 1; l <= N; l++) {
					for (int kk = -l; kk <= l; kk++) {
						mult = powi(l - n + abs(m - kk))
								* pow(eiphi, double(m - kk))
								* bessj.t[int(abs(m - kk))][k] * cstMult;
						rppp = app.t[0] + powm1(l - kk) * app.t[1];
						rppm = app.t[0] - powm1(l - kk) * app.t[1];
						rptp = apt.t[0] + powm1(l - kk) * apt.t[1];
						rptm = apt.t[0] - powm1(l - kk) * apt.t[1];
						...
						PP = legPi.t[k + 2 * layJ * Ng][ii(n, m)]
								* legPi.t[k + 2 * layI * Ng][ii(l, kk)];
						TT = legTau.t[k + 2 * layJ * Ng][ii(n, m)]
								* legTau.t[k + 2 * layI * Ng][ii(l, kk)];
						PT = legPi.t[k + 2 * layJ * Ng][ii(n, m)]
								* legTau.t[k + 2 * layI * Ng][ii(l, kk)];
						TP = legTau.t[k + 2 * layJ * Ng][ii(n, m)]
								* legPi.t[k + 2 * layI * Ng][ii(l, kk)];
...
void Reflection::calcVSWFsIntegral(const double &dx, const double &dy,
		const double &zzi, const double &zzj) {
	Mmn = Nmn = Matrix(NM, 3, 0.);
	Vector app, asp, apt, ast, a(2, 1.);
	Complex rpp, rpt, rsp, rst;
	Complex cost, mult, multExp, multExp1, multExp2;
	...
	for (int k = 0; k < 2 * Ng; k++) {
		...
		elmtI = k + 2 * layI * Ng;
		bes.assign(N + 1, rauij * kappa.t[k]);
		mult = -gaussLeg.t[k][1] * kappa.t[k] * dkappa.t[k] / ml.ki.t[layJ]
				/ kz.t[elmtJ];
		cosb = kz.t[elmtI] / ml.ki.t[layI];
		int i = 0;
		for (int n = 1; n <= N; n++) {
			for (int m = -n; m <= n; m++) {
				...
				multExp = mult * bes.J.t[int(abs(m))] * powi(-n + abs(m) - 1)
						* pow(eiphi, double(m));
				multExp1 = mult * bes.J.t[int(abs(m + 1))]
						* powi(-n + abs(m + 1) - 1) * pow(eiphi, double(m + 1));
				multExp2 = mult * bes.J.t[int(abs(m - 1))]
						* powi(-n + abs(m - 1) - 1) * pow(eiphi, double(m - 1));

				Mmn.t[i][0] += 0.5
						* ((multExp1 + multExp2) * legPi.t[elmtJ][i]
								* (app.t[0] - app.t[1]) * cosb
								- (multExp1 - multExp2) * legTau.t[elmtJ][i]
										* (ast.t[0] + ast.t[1]));
				Mmn.t[i][1] += 0.5
						* ((multExp1 - multExp2) / i_ * legPi.t[elmtJ][i]
								* (app.t[0] - app.t[1]) * cosb
								+ (multExp1 + multExp2) * i_
										* legTau.t[elmtJ][i]
										* (ast.t[0] + ast.t[1]));
				Mmn.t[i][2] += multExp
						* ((app.t[0] + app.t[1]) * legPi.t[elmtJ][i]
								* sqrt(1. - cosb * cosb));

				Nmn.t[i][0] += 0.5
						* ((multExp1 + multExp2) * legTau.t[elmtJ][i]
								* (apt.t[0] - apt.t[1]) * cosb
								- (multExp1 - multExp2) * legPi.t[elmtJ][i]
										* (asp.t[0] + asp.t[1]));
				Nmn.t[i][1] += 0.5
						* ((multExp1 - multExp2) / i_ * legTau.t[elmtJ][i]
								* (apt.t[0] - apt.t[1]) * cosb
								+ (multExp1 + multExp2) * i_ * legPi.t[elmtJ][i]
										* (asp.t[0] + asp.t[1]));
				Nmn.t[i][2] += multExp
						* (legTau.t[elmtJ][i] * (apt.t[0] + apt.t[1])
								* sqrt(1. - cosb * cosb));
				i++;
			}
		}
...
void Reflection1::setConstants(const double &lambd, const int &n,
		const int &ng) {
	if (N != n)
		cg.assign(n);
	if ((N != n) || (Ng != ng) || (lambd != lambda)) {
		...
		for (int l = 0; l < nbrLay; l++) {
			for (int i = 0; i < Ng; i++) {
				...
				for (int u = -2; u <= 2; u += 2) {
					for (int w = abs(u); w <= 2 * N; w++) {
						for (int v = 0; v <= w; v++) {
							duvw.t[i + Ng + 2 * l * Ng][elmt] = wign.dlmn(u, v,
									w);
							elmt++;
						}
					}
...
void Reflection1::setParameters(const double &xx, const double &yy,
		const double &zzi, const double zzj, const bool &self) {
	lay = ml.getMedium(zi);
...
Complex Reflection1::getQelmt(const int &u, const int &v, const int &w,
		const int &pt, const int &pm) {
	if (w < abs(u) || w < v)
		return 0.;
	int i = 0, elmt;
	for (int U = -2; U <= 2; U += 2) {
...
void Reflection1::calcQintegrals() {
	...
	for (int k = 0; k < 2 * Ng; k++) {
		...
		elmt = 0;
		mult1 = gaussLeg.t[k][1] * kappa.t[k] * dkappa.t[k] / ml.ki.t[layJ]
				/ kz.t[k + 2 * layJ * Ng];
		for (int u = -2; u <= 2; u += 2) {
			for (int w = abs(u); w <= 2 * N; w++) {
				for (int v = 0; v <= w; v++) {
					mult = mult1 * powi(abs(v)) * bessj.t[int(abs(v))][k]
							* duvw.t[k + 2 * layJ * Ng][elmt];
					Q1.t[elmt] += mult
							* ((ap1.t[0] + powi(u) * as2.t[0])
									+ (-ap1.t[1] + powi(u) * as2.t[1]));
					Q2.t[elmt] += mult
							* ((ap1.t[0] + powi(u) * as2.t[0])
									- (-ap1.t[1] + powi(u) * as2.t[1]));
					Q3.t[elmt] += mult
							* ((ap2.t[0] + powi(u) * as1.t[0])
									+ (-ap2.t[1] + powi(u) * as1.t[1]));
					Q4.t[elmt] += mult
							* ((ap2.t[0] + powi(u) * as1.t[0])
									- (-ap2.t[1] + powi(u) * as1.t[1]));
					elmt++;
...
void Reflection1::calcQintegralsSelf() {
	...
	for (int k = 0; k < 2 * Ng; k++) {
		...
		elmt = 0;
		mult1 = gaussLeg.t[k][1] * kappa.t[k] * dkappa.t[k] / ml.ki.t[lay]
				/ kz.t[k + 2 * lay * Ng];
		for (int u = -2; u <= 2; u += 2) {
			for (int w = abs(u); w <= 2 * N; w++) {
				for (int v = 0; v <= w; v++) {
...
void Reflection1::calcRSelf() {
	...
	for (int n = 1; n <= N; n++) {
		for (int m = -n; m <= n; m++) {
			...
			for (int l = 1; l <= N; l++) {
				for (int k = -l; k <= l; k++) {
					mult = 0.25 * powm1(m) * powi(l - n)
							* sqrt(double(n + n + 1) * double(l + l + 1));
					if (m == k) {
						for (int w = abs(n - l); w <= n + l; w++) {
							C1 = cg.coef(w, n, m, l, -k)
									* cg.coef(w, n, 1, l, -1);
							C2 = cg.coef(w, n, m, l, -k)
									* cg.coef(w, n, 1, l, 1);
							pm1 = powm1(n + l + w);
							...
							R.t[line][col] += mult
									* (C1 * (1. + pm1) * Qa0
											+ C2 * (Qap2 + pm1 * Qam2));
							R.t[line][col + NM] += mult
									* (C1 * (1. - pm1) * Qb0
											- C2 * (Qbp2 - pm1 * Qbm2));
							R.t[line + NM][col] += mult
									* (C1 * (1. - pm1) * Qc0
											+ C2 * (Qcp2 - pm1 * Qcm2));
							R.t[line + NM][col + NM] += mult
									* (C1 * (1. + pm1) * Qd0
											- C2 * (Qdp2 + pm1 * Qdm2));
						}
					}
...
void Reflection1::calcR() {
	...
	for (int n = 1; n <= N; n++) {
		for (int m = -n; m <= n; m++) {
			...
			for (int l = 1; l <= N; l++) {
				for (int k = -l; k <= l; k++) {
					mult = 0.25 * powm1(k + 1) * powi(l - n)
							* pow(eiphi, double(m - k))
							* sqrt(double(n + n + 1) * double(l + l + 1));
					...
					for (int w = abs(n - l); w <= n + l; w++) {
						...
						if (m - k < 0) {
							Qam2 = powm1(m - k)
									* getQelmt(2, k - m, w, n - m, l - k + 1);
							Qa0 = powm1(m - k)
									* getQelmt(0, k - m, w, n - m, l - k + 1);
							Qap2 = powm1(m - k)
									* getQelmt(-2, k - m, w, n - m, l - k + 1);

							Qbm2 = powm1(m - k)
									* getQelmt(2, k - m, w, n - m, l - k);
							Qb0 = powm1(m - k)
									* getQelmt(0, k - m, w, n - m, l - k);
							Qbp2 = powm1(m - k)
									* getQelmt(-2, k - m, w, n - m, l - k);

							Qcm2 = powm1(m - k)
									* getQelmt(2, k - m, w, n - m + 1,
											l - k + 1);
							Qc0 = powm1(m - k)
									* getQelmt(0, k - m, w, n - m + 1,
											l - k + 1);
							Qcp2 = powm1(m - k)
									* getQelmt(-2, k - m, w, n - m + 1,
											l - k + 1);
							...
							Qdm2 = powm1(m - k)
									* getQelmt(2, k - m, w, n - m + 1, l - k);
							Qd0 = powm1(m - k)
									* getQelmt(0, k - m, w, n - m + 1, l - k);
							Qdp2 = powm1(m - k)
									* getQelmt(-2, k - m, w, n - m + 1, l - k);
						...
						Ri.t[line][col] += powm1(m + k) * mult
								* (C1 * (1. + pm1) * Qa0
										+ C2 * (Qap2 + pm1 * Qam2));
						Ri.t[line][col + NM] += powm1(m + k) * mult
								* (C1 * (1. - pm1) * Qb0
										- C2 * (Qbp2 - pm1 * Qbm2));
						Ri.t[line + NM][col] += powm1(m + k) * mult
								* (C1 * (1. - pm1) * Qc0
										+ C2 * (Qcp2 - pm1 * Qcm2));
						Ri.t[line + NM][col + NM] += powm1(m + k) * mult
								* (C1 * (1. + pm1) * Qd0
										- C2 * (Qdp2 + pm1 * Qdm2));
					}
					col++;
...
void Reflection1::calcVSWFsIntegral(const double &dx, const double &dy,
		const double &zzi, const double &zzj) {
	Mmn = Nmn = Matrix(NM, 3, 0.);
	Vector app, asp, apt, ast, a(2, 1.);
	Complex rpp, rpt, rsp, rst;
