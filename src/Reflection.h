...
public:
	Reflection();
	void setParameters(const double &xx, const double &yy, const double &zzi,
	...
	Matrix calcR();
	Matrix calcRSelf();
	void calcVSWFsIntegral(const double &dx, const double &dy,
...
public:
	...
	void calcRSelf();
	int nbrQelmts();
	Complex getQelmt(const int &u, const int &v, const int &w, const int &pt,
			const int &pm);
