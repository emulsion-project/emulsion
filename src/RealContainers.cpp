...
RVector::RVector(const int &n) :
		n(n), t(n > 0 ? new double[n] : NULL) {
}
RVector::RVector(const int &n, const double &x) :
		n(n), t(n > 0 ? new double[n] : NULL) {
	for (int i = 0; i < n; i++)
		t[i] = x;
}
RVector::RVector(const RVector &v) :
		n(v.n), t(n > 0 ? new double[n] : NULL) {
	for (int i = 0; i < n; i++)
		t[i] = v.t[i];
}
...
RMatrix::RMatrix(const int &nn, const int &mm) :
		n(nn), m(mm), t(nn > 0 ? new double*[nn] : NULL) {
	int mn = mm * nn;
	if (t)
		t[0] = mn > 0 ? new double[mn] : NULL;
	for (int i = 1; i < nn; i++)
		t[i] = t[i - 1] + mm;
}

RMatrix::RMatrix(const int &nn, const int &mm, const double &x) :
		n(nn), m(mm), t(n > 0 ? new double*[nn] : NULL) {
	int mn = mm * nn;
	if (t)
		t[0] = mn > 0 ? new double[mn] : NULL;
	for (int i = 1; i < nn; i++)
...
RMatrix::RMatrix(const RMatrix &M) :
		n(M.n), m(M.m), t(n > 0 ? new double*[n] : NULL) {
	int mn = n * m;
	if (t != NULL)
		t[0] = mn > 0 ? new double[mn] : NULL;
	for (int i = 1; i < n; i++)
