...
LegendreFunctions::LegendreFunctions() :
		N(0), pmn(NULL), pimn(NULL), taumn(NULL) {
}

LegendreFunctions::LegendreFunctions(const int &NN, const double &xx) :
		N(NN), x(xx) {
	if (NN >= 0) {
...
void LegendreFunctions::calcElmts() {
	...
	for (i = 2; i <= N; i++) {
		pmn[0].t[i] = sqrt((4. * double(i * i) - 1.) / double(i * i))
				* (x * pmn[0].t[i - 1]
						- sqrt(
								(double(i - 1) * double(i - 1))
										/ (4. * double(i - 1) * double(i - 1)
												- 1.)) * pmn[0].t[i - 2]);
		pimn[0].t[i] = pmn[0].t[i] / sint;
		taumn[0].t[i] = (double(i) * x * pmn[0].t[i]
				- sqrt(double(2 * i + 1) * double(i * i) / double(2 * i - 1))
						* pmn[0].t[i - 1]) / sint;
	}
	for (i = 1; i <= N; i++) {
		pmn[i].t[0] = sqrt(double(2 * i + 1) / double(2 * i)) * sint
				* pmn[i - 1].t[0];
		...
		if (N > i) {
			...
			taumn[i].t[1] = (double(i + 1) * x * pmn[i].t[1]
					- sqrt(
							double(2 * (i + 1) + 1)
									* double((i + 1) * (i + 1) - i * i)
									/ double(2 * (i + 1) - 1)) * pmn[i].t[0])
					/ sint;
		}
		for (j = 2; j <= N - i; j++) {
			pmn[i].t[j] = sqrt(
...
void LegendreFunctions::calcElmtsPi() {
	...
	for (i = 1; i <= N; i++) {
		...
		for (j = 2; j <= N - i; j++) {
			pmn[i].t[j] = 0.;
			if (i == 1) {
				pimn[i].t[j] = powm1(j + 2)
						* sqrt(
								double((j + 1) * (j + 1) + (j + 1))
										* double((j + 1) + (j + 1) + 1) * 0.5)
						* 0.5;
				taumn[i].t[j] = -powm1(j + 2)
						* sqrt(
								double((j + 1) * (j + 1) + (j + 1))
										* double((j + 1) + (j + 1) + 1) * 0.5)
						* 0.5;
...
LegendreFunctionsC::LegendreFunctionsC(const int &NN, const Complex &xx) :
		N(NN), x(xx) {
	if (NN >= 0) {
		pmn = new Vector[NN + 1];
