...
Matrix::Matrix(const int &nn, const int &mm) :
		n(nn), m(mm), t(nn > 0 ? new Complex*[nn] : NULL) {
	int mn = mm * nn;
	if (t)
		t[0] = mn > 0 ? new Complex[mn] : NULL;
	for (int i = 1; i < nn; i++)
		t[i] = t[i - 1] + mm;
}

Matrix::Matrix(const int &nn, const int &mm, const Complex &x) :
		n(nn), m(mm), t(n > 0 ? new Complex*[nn] : NULL) {
	int mn = mm * nn;
	if (t)
		t[0] = mn > 0 ? new Complex[mn] : NULL;
	for (int i = 1; i < nn; i++)
...
Matrix::Matrix(const Matrix &M) :
		n(M.n), m(M.m), t(n > 0 ? new Complex*[n] : NULL) {
	int mn = n * m;
	if (t != NULL)
		t[0] = mn > 0 ? new Complex[mn] : NULL;
	for (int i = 1; i < n; i++)
...
Complex& Matrix::operator()(const int &i, const int &j) {
	return t[i][j];
}
...
bool gaussj(Matrix &a, Matrix &b) {
	...
	for (i = 0; i < n; i++) {
		...
		if (irow != icol) {
			for (l = 0; l < n; l++)
				swap(a.t[irow][l], a.t[icol][l]);
			for (l = 0; l < m; l++)
				swap(b.t[irow][l], b.t[icol][l]);
		}
		indxr[i] = irow;
		indxc[i] = icol;
		if (a.t[icol][icol] == 0.)
		...
		for (ll = 0; ll < n; ll++) {
			if (ll != icol) {
				...
				a.t[ll][icol] = 0.0;
				for (l = 0; l < n; l++)
					a.t[ll][l] -= a.t[icol][l] * dum;
				for (l = 0; l < m; l++)
					b.t[ll][l] -= b.t[icol][l] * dum;
			}
		}
	}
	for (l = n - 1; l >= 0; l--)
		if (indxr[l] != indxc[l])
			for (k = 0; k < n; k++)
				swap(a.t[k][indxr[l]], a.t[k][indxc[l]]);
	return true;
}
...
void hessenberg(Matrix &A) {
	...
	for (i = 0; i < n - 2; i++) {
		max = 0.;
		for (j = i + 1; j < n; j++)
			if (abs(max) < abs(A.t[j][i])) {
				max = A.t[j][i];
				piv = j;
			}
		if (piv != i + 1) {
			for (j = i; j < n; j++)
				swap(A.t[piv][j], A.t[i + 1][j]);
			for (j = 0; j < n; j++)
				swap(A.t[j][piv], A.t[j][i + 1]);
		}
		if (abs(max) > 0.) {
			for (j = i + 2; j < n; j++) {
				if (abs(A.t[j][i]) > 0.) {
					ri = A.t[j][i] / max;
					for (k = i; k < n; k++)
						A.t[j][k] -= ri * A.t[i + 1][k];
					for (k = 0; k < n; k++)
						A.t[k][i + 1] += ri * A.t[k][j];
				}
			}
		}
	}
...
void qr(Matrix &A) {
	...
	imin = 0;
	for (k = 0; k < 10 * m; k++) {
		while ((imax > imin) && (abs(A.t[imax][imax - 1]) < 1e-15))
			imax--;
		if (imax == imin)
			break;
		...
		a4 = A.t[imax][imax];
		lambda1 = (a1 + a4) * 0.5
				+ sqrt((a1 - a4) * (a1 - a4) * 0.25 + a2 * a3);
		lambda = (a1 + a4) * 0.5 - sqrt((a1 - a4) * (a1 - a4) * 0.25 + a2 * a3);
		if (abs(a4 - lambda1) < abs(a4 - lambda))
			lambda = lambda1;
		for (i = 0; i <= imax; i++)
			A.t[i][i] -= lambda;
		for (i = 0; i < imax; i++) {
	...
	for (i = 0; i < m - 1; i++) {
		for (j = i; j < m - 1; j++) {
			...
			for (k = 0; k < m; k++)
				A.t[i][k] += ri * A.t[j + 1][k];
			for (k = 0; k < m; k++)
				A.t[k][j + 1] -= ri * A.t[k][i];
		}
	}
}
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0; j < Y.n; j++) {
			...
			for (int ii = 0; ii < Y.n; ii++)
				if (ii != j)
					a *= (B.t[i] - XX.t[ii]) / (XX.t[j] - XX.t[ii]);
			PA.t[i][1] += a;
		}
		for (int j = 0; j < m; j++)
			if (j != i)
				PA.t[i][1] /= B.t[i] - B.t[j];
		//PA.t[i][1] *= B.t[i];
	}
...
void gmres(Vector &X, void *Struct, Vector (*mult)(void*, const Vector&),
		const double &tol) {
	const int kmax = 1000;
