...
void IncidentRadiation::setParameters(const int &inc, const double &lambd) {
  ...
  if (type == 0) { // plane wave
    ...
    if (beta0<=piDbl/2.) {
      aincs.t[0] = Ea;
      aincp.t[0] = Eb;
      ml.setKxy(ml.ki.t[0]*sqrt(cos(alpha0)*sin(beta0)*cos(alpha0)*sin(beta0)+sin(alpha0)*sin(beta0)*sin(alpha0)*sin(beta0)));
    }
    else {
      aincs.t[1] = Ea;
      aincp.t[1] = Eb;
      ml.setKxy(ml.ki.t[ml.nbrMediums-1]*sqrt(cos(alpha0)*sin(beta0)*cos(alpha0)*sin(beta0)+sin(alpha0)*sin(beta0)*sin(alpha0)*sin(beta0)));
    }
...
void IncidentRadiation::setPlaneWaveCoeffs(const int &n, const Vector3d &pos) {
  ...
  ap = exp(i_*ml.kxy*(pos.t[0]*cos(alpha0)+pos.t[1]*sin(alpha0)))*ap;
  as = exp(i_*ml.kxy*(pos.t[0]*cos(alpha0)+pos.t[1]*sin(alpha0)))*as;
  ...
  for (int n=1 ; n<=N ; n++) {
    ...
    for (int m=-n ; m<=n ; m++) {
      ...
      Amn.t[i] = mult*(i_*double(m)*leg.Pimn(n,m)*ap.t[0] + leg.Taumn(n,m)*as.t[0]);
      Amn.t[i+NM] = mult*i_*(leg.Taumn(n,m)*ap.t[0] - i_*double(m)*leg.Pimn(n,m)*as.t[0]);
      i++;
    }
  }
  ...
  for (int n=1 ; n<=N ; n++) {
    ...
    for (int m=-n ; m<=n ; m++) {
      mult = multn*exp(-i_*double(m)*alpha0);
      Amn.t[i] += mult*(i_*double(m)*leg.Pimn(n,m)*ap.t[1] + leg.Taumn(n,m)*as.t[1]);
      Amn.t[i+NM] += mult*i_*(leg.Taumn(n,m)*ap.t[1] - i_*double(m)*leg.Pimn(n,m)*as.t[1]);
      i++;
    }
  }
  return;
...
void IncidentRadiation::setElectron(const int &n, const double &e0, const double &ee, const double &x, const double &y) {
  N = n;
  int NM = (N+1)*(N+1)-1;
  ...
  for (int n=1 ; n<=N ; n++) {
    for (int m=-n ; m<=n ; m++) {
      mult = -2.*piDbl*powi(1-n)*omega/(c*c*double(n*n+n))*besselKn(m,omega*b/(v0*gamma))*exp(-i_*double(m)*phi0-i_*omega*z0/v0);
