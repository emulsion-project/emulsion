...
RefractiveIndex::RefractiveIndex(const int &mat,const double &r) {
  ...
  else if (type == 1) ifs >> fileName;
  else if (type == 2) ifs >> wP >> gamma0;
  else if (type == 3) ifs >> wP >> gamma0 >> A >> vF >> fileName;
}
...
Complex RefractiveIndex::calcIndex(const double &lambd) {
  lambda = lambd;
  if (type == 0) return n;
  else if (type == 1) return interpolation();
  else if (type == 2) return drude();
  return drudeMod();
}
