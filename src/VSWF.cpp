...
VSWF::VSWF(const int &NN, const Complex &KR, const double &ct, const double &p) {
  N = NN;
  kr = KR;
  cost = ct;
  phi = p;
...
void VSWF::setParameters(const int &NN, const Complex &KR, const double &ct, const double &p) {
  N = NN;
  kr = KR;
  cost = ct;
  phi = p;
...
void VSWF::calcVSWFs(const int &m, const int &n, const int &i, const bool &calcExp) {
  mult = 1./sqrt(2.*double(n*n+n));
  if (calcExp) mult *= exp(i_*double(m)*phi);
  Mmn.t[0] = 0.;
  if (i==1) {
...
VSWFsub::VSWFsub(const int &NN, const Complex &KM1,const Complex &KM2, const double &X, const double &Xj, const double &Y, const double &Yj, const double &Z, const double &Zj, const double &zsub) {
  N = NN;
  km1 = KM1;
  km2 = KM2;
  x = X; xj = Xj; y = Y; yj = Yj; z = Z; zj = Zj, zSub = zsub;
...
void VSWFsub::setParameters(const int &NN, const Complex &KM1,const Complex &KM2, const double &X, const double &Xj, const double &Y, const double &Yj, const double &Z, const double &Zj, const double &zsub) {
  N = NN;
  km1 = KM1;
  km2 = KM2;
  x = X; xj = Xj; y = Y; yj = Yj; z = Z; zj = Zj, zSub = zsub;
...
void VSWFsub::calcVSWFsRefl(const int &m, const int &n) {
  ...
  for (int kb=0 ; kb<Ngb ; kb++) {
    ...
    for (int ka=0 ; ka<Nga ; ka++) {
      ...
      mult *= -1./(2.*piDbl*powi(n+1)*sqrt(2*n*(n+1)));
      Mmn.t[0] += mult*(-double(m)*legR[kb].Pimn(n,m)*t*cos(alpha)*rp-i_*legR[kb].Taumn(n,m)*sin(alpha)*rs);
      Mmn.t[1] += mult*(double(m)*legR[kb].Pimn(n,m)*t*sin(alpha)*rp+i_*legR[kb].Taumn(n,m)*cos(alpha)*rs);
      Mmn.t[2] -= mult*double(m)*legR[kb].Pimn(n,m)*sqrt(1.-t*t)*rp;

      Nmn.t[0] += mult*(legR[kb].Taumn(n,m)*t*cos(alpha)*rp-i_*double(m)*legR[kb].Pimn(n,m)*sin(alpha)*rs);
      Nmn.t[1] += mult*(legR[kb].Taumn(n,m)*t*sin(alpha)*rp+i_*double(m)*legR[kb].Pimn(n,m)*cos(alpha)*rs);
...
void VSWFsub::calcVSWFsTrans(const int &m, const int &n) {
  ...
  for (int kb=0 ; kb<Ngb ; kb++) {
    ...
    for (int ka=0 ; ka<Nga ; ka++) {
      ...
      mult *= gaussLeg.t[ka][1]*gaussLag.t[kb][1]*exp(q)/q;
      mult *= -1./(2.*piDbl*powi(n+1)*sqrt(double(2*n*(n+1))));
      Mmn.t[0] += mult*(-double(m)*legT[kb].Pimn(n,m)*t*cos(alpha)*tp-i_*legT[kb].Taumn(n,m)*sin(alpha)*ts);
      Mmn.t[1] += mult*(double(m)*legT[kb].Pimn(n,m)*t*sin(alpha)*tp+i_*legT[kb].Taumn(n,m)*cos(alpha)*ts);
      Mmn.t[2] -= mult*double(m)*legT[kb].Pimn(n,m)*sqrt(1.-t*t)*tp;

      Nmn.t[0] += mult*(legT[kb].Taumn(n,m)*t*cos(alpha)*tp-i_*double(m)*legT[kb].Pimn(n,m)*sin(alpha)*ts);
      Nmn.t[1] += mult*(legT[kb].Taumn(n,m)*t*sin(alpha)*tp+i_*double(m)*legT[kb].Pimn(n,m)*cos(alpha)*ts);
...
VSWFsubinf::VSWFsubinf(const int &NN, const Complex &KM1,const Complex &KM2, const double &t, const double &p, const double &Zj) {
  N = NN;
  km1 = KM1;
  km2 = KM2;
  theta = t;
...
void VSWFsubinf::setParameters(const int &NN, const Complex &KM1,const Complex &KM2, const double &t, const double &p, const double &Zj) {
  N = NN;
  km1 = KM1;
  km2 = KM2;
  theta = t;
...
VSWFds::VSWFds(const int &NN, const Complex &K, const double &rr, const Vector &Sources, const double &ct, const double &p) {
  N = NN;
  k = K;
  r = rr;
  sources = Sources;
...
void VSWFds::setParameters(const int &NN, const Complex &K, const double &rr, const Vector &Sources, const double &ct, const double &p) {
  N = NN;
  k = K;
  r = rr;
  sources = Sources;
...
void VSWFds::calcVSWFsds(const int &m,const int &n,const int &i,const bool &calcExp) {
  int nn = abs(m);
  if (nn == 0) nn = 1;
...
Vector3d calcMmnds(const int &m, const int &i, const Complex &k, const double &r, const Complex &zn, const double &ct) {
  int n = abs(m);
  if (m==0) n = 1;
  int NBes = n;
  double mr = double(m);
...
Vector3d calcNmnds(const int &m, const int &i, const Complex &k, const double &r, const Complex &zn, const double &ct) {
  int n = abs(m);
  if (m==0) n = 1;
  int NBes = n;
  double mr = double(m);
