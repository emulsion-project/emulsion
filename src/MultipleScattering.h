...
public:
	...
	double calcCscaN(const bool &normalize, const double &nor, const int &nn,
			const int &pol);
	RVector calcCextN(const bool &normalize, const double &nor, const int &nn,
			const int &pol);
	RVector calcCabsN(const bool &normalize, const double &nor, const int &nn,
			const int &pol);
	Vector calcComplexCext(const bool &normalize, const double &nor);
	Vector calcComplexCextN(const bool &normalize, const double &nor,
			const int &nn, const int &pol);
	Vector calcStokesParametersInc();
	Vector calcStokesParametersExt();
	Matrix calcScatteringMatrix(const double &ctp, const double &ctm,
			const double &alphap, const double &alpham);
	...
	Vector3d calcEint(const int &part, const double &xx, const double &yy,
			const double &zz);
	Vector3d calcHint(const int &part, const double &xx, const double &yy,
			const double &zz);
	...
	Reflection1 refl1;
	Multilayer1 ml;
	int Np, Npx, Npy, Npz, N, NM, solver, Ng, methodTMatrix, periodic, nbrDim,
			nMatFFT, nMatFFTx, nMatFFTy, lay, nMatrices;
