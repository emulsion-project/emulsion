...
Vector Multilayer1::sourceExtIn(const Vector &a, const int &pol, const double &z) {
  Vector aPM(2,0.),aa;
  int lay = getMedium(z);
  if (lay == 0) {
...
Vector Multilayer1::sourceInt(const int &pol, const Vector &a, const double &zs, const double &z) { // a = aP & aM
  aS=Vector(2,0.);
  Vector aSi(2,0.);
  Vector aa(2,0.);
...
Matrix Multilayer1::calcSMatrixInt(const int &pol,const double &z1, const double &z2) { // 0:TE 1:TM
  Matrix S,Su,Sl;
  S = Su = Sl = Matrix(2,2,0.);
