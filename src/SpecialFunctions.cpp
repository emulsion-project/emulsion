...
Complex fresnelCoefRP(const Complex &m, const Complex &cosBeta1) {
  ...
  if (imag(cosBeta2*m) < 0.) cosBeta2 = -cosBeta2;
  if ((m*cosBeta1-cosBeta2) < 1e-14) return 0.;
...
Complex fresnelCoefRS(const Complex &m, const Complex &cosBeta1) {
  ...
  cosBeta2 = sqrt(1.-sinBeta2*sinBeta2);
  if (imag(cosBeta2*m) < 0.) cosBeta2 = -cosBeta2;
  if ((cosBeta1-m*cosBeta2) <= 1e-14) return 0.;
...
Complex fresnelCoefTP(const Complex &m, const Complex &cosBeta1) {
  ...
  sinBeta2 = sinBeta1/m;
  cosBeta2 = sqrt(1.-sinBeta2*sinBeta2);
  if (imag(cosBeta2) < 0.) cosBeta2 = -cosBeta2;
  if ((cosBeta1+m*cosBeta2) < 1e-14) return 0.;
...
double gegenbauer(const double &alph, const int &n, const double &x) {
  if (n == 0) return 1.;
  if (n == 1) return 2.*alph*x;
  double Cn=0.,Cnm1,Cnm2;
  Cnm2 = 1.;
  Cnm1 = 2.*alph*x;
  for (int i=2 ; i<=n ; i++) {
    Cn = (2.*x*(double(i-1)+alph)*Cnm1 - (double(i-2)+2.*alph)*Cnm2)/double(i);
    Cnm2 = Cnm1;
    Cnm1 = Cn;
  }
  return Cn;
...
Complex Amnplus(const int &mm, const int &n, const double &vc) {
  int m = abs(mm);
  if (m>n) return 0.;
  Complex Ap = powi(n+m)*sqrt(double(n+n+1)*fact(n-m)/(piDbl*fact(n+m)))*dblfact(2*m-1)/pow(vc/sqrt(1.-vc*vc),m)*gegenbauer(m+0.5,n-m,1./vc)/vc;
  if (mm < 0) return powm1(m)*Ap;
  return Ap;
}

Complex Bmnplus(const int &mm, const int &n, const double &vc) {
  int m = abs(mm);
  Complex Bp = Amnplus(m+1,n,vc)*sqrt(double(n+m+1)*double(n-m)) - Amnplus(m-1,n,vc)*sqrt(double(n-m+1)*double(n+m));
  if (mm < 0) return powm1(m)*Bp;
  return Bp;
}
