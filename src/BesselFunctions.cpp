...
void BesselFunctions::calcCJ(const int &N, const double &alpha,
		const Complex &zz) {
	z = zz;
	if (z.im < 0.)
		z.im *= -1.;
	...
	for (n = 1; n <= eta - 1; n++) {
		...
		lambda.t[n + 1] = 2. * powi(-n - 1) * (alpha + double(n + 1))
				* l.t[n + 1];
	}
...
void BesselFunctions::calcJ(const int &N, const double &alpha,
		const double &xx) {
	x = xx;
	if (x < 0.)
...
Complex SphericalBessel::jn(const int &n) {
	return J.t[n];
}
Complex SphericalBessel::yn(const int &n) {
	return Y.t[n];
}
Complex SphericalBessel::h1n(const int &n) {
	return J.t[n] + i_ * Y.t[n];
}
Complex SphericalBessel::h2n(const int &n) {
	return J.t[n] - i_ * Y.t[n];
}
Complex SphericalBessel::djn(const int &n) {
	return dJ.t[n];
}
Complex SphericalBessel::dyn(const int &n) {
	return dY.t[n];
}
Complex SphericalBessel::dh1n(const int &n) {
	return dJ.t[n] + i_ * dY.t[n];
}
Complex SphericalBessel::dh2n(const int &n) {
	return dJ.t[n] - i_ * dY.t[n];
}
...
Bessel::Bessel(const int &NN, const Complex &z) {
	...
	for (n = 0; n <= NN; n++)
		J.t[n] = j.t[n] / (2. * ll - j.t[0]);
	for (n = nn - 1; n >= 0; n--)
		j.t[n] /= 2. * ll - j.t[0];

	Y.t[0] = 2. / piDbl * (log(0.5 * z) + euler) * J.t[0];
	for (n = 1; n <= nn / 2 - 1; n++)
		Y.t[0] -= 4. / piDbl * powm1(n) * j.t[n * 2] / double(n);
	for (n = 1; n <= NN; n++)
		Y.t[n] = (Y.t[n - 1] * J.t[n] - 2. / (piDbl * z)) / J.t[n - 1];
}
...
Complex besselJn(const int &n, const Complex &z) {
	...
	jn = 0.;
	for (int j = 0; j < i; j++)
		jn += wx.t[j][1] * cos(z * sin(wx.t[j][0]) - double(n) * wx.t[j][0]);
	jn /= piDbl;
	jnn = jn;
	return jn;
}
