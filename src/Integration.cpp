...
RMatrix gaussLagQuad(const int &n, const double &alf) {
	...
	for (i = 0; i < n; i++) {
		if (i == 0)
			z = (1. + alf) * (3. + 0.92 * alf)
					/ (1. + 2.4 * double(n) + 1.8 * alf);
		else if (i == 1)
			z += (15. + 6.25 * alf) / (1. + 0.9 * alf + 2.5 * double(n));
		else {
			ai = i - 1;
			z += ((1. + 2.55 * double(ai)) / (1.9 * double(ai))
					+ 1.26 * double(ai) * alf / (1. + 3.5 * double(ai)))
					* (z - x.t[i - 2][0]) / (1. + 0.3 * alf);
		}
		for (its = 0; its < maxit; its++) {
			...
			for (j = 0; j < n; j++) {
				p3 = p2;
				p2 = p1;
				p1 = ((double(j + j + 1) + alf - z) * p2
						- (double(j) + alf) * p3) / double(j + 1);
			}
		...
		x.t[i][0] = z;
		x.t[i][1] = -exp(gammaln(alf + double(n)) - gammaln(double(n)))
				/ (pp * double(n) * p2);
	}
	return x;
}
