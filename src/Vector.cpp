...
Vector::Vector() : n(0), t(NULL) {}
Vector::Vector(const int &n) : n(n), t(n>0? new Complex[n] : NULL) {}
Vector::Vector(const int &n,const Complex &x) : n(n), t(n>0 ? new Complex[n] : NULL) { for (int i=0 ; i<n ; i++) t[i] = x; }
Vector::Vector(const Vector &v) : n(v.n), t(n>0 ? new Complex[n] : NULL) { for (int i=0 ; i<n ; i++) t[i] = v.t[i]; }

/*********************************
 *     Class destructor          *
 *********************************/

Vector::~Vector() { if(t!=NULL) delete[] (t); }
...
Vector operator+(const Vector &v1,const Vector &v2) {
  Vector v3(v1.n);
  if (v1.n == v2.n) for (int i=0 ; i<v3.n ; i++) v3.t[i] = v1.t[i]+v2.t[i];
  return v3;
}
...
Complex operator*(const Vector &v1,const Vector &v2) {
  Complex t=0.;
  if (v1.n == v2.n) for (int i=0 ; i<v1.n ; i++) t += conj(v1.t[i])*v2.t[i];
  return t;
}

Vector operator*(const Vector &v1,const Complex &z) {
  Vector v2(v1.n);
  for(int i=0 ; i<v2.n ; i++) v2.t[i] = v1.t[i] * z;
  return v2;
}
...
Complex max(const Vector &V) {
  Complex v = V.t[0];
  for (int i=0 ; i<V.n ; i++) if (v < V.t[i]) v = V.t[i];
  return v;
}
...
Vector poleAmplitude(const Vector &x, const Vector &y) {
  ...
  for (int i=0 ; i<n ; i++) {
      ...
      for (int j=0 ; j<n ; j++) {
          if (j!=i) a *= (xpap.t[0] - x.t[j])/(x.t[i] - x.t[j]);
      }
      aa += a;
  }
  xpap.t[1] = aa;
...
double norm(const Vector &v) {
  double x = 0.;
  for (int i=0 ; i<v.n ; i++) x += norm(v.t[i]);
  return sqrt(x);
}
...
Vector fft(const Vector &v, const int &isign) {
  ...
  double mult = 1.;
  if (isign == -1) mult = 1./double(n);
  transf.t[0] = Complex(data[0]*mult,data[1]*mult);
  for (i=1 ; i<n ; i++) transf.t[i] = mult*Complex(data[2*n-1-2*(i-1)-1],data[2*n-1-2*(i-1)]);
  delete [] data;
  return transf;
}
