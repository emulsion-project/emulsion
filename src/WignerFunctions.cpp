...
WignerFunctions::WignerFunctions(const int &nn, const double &thet) {
	...
	for (m = -N; m <= N; m++) {
		...
		for (l = -N; l <= N; l++) {
			...
			ee = l < m ? powm1(m - l) : 1.;
			coefs[row][col].t[0] = d = ee * pow(2., -double(n0))
					* sqrt(
							exp(
									lns[n0 + n0] - lns[int(abs(m - l))]
											- lns[int(abs(m + l))]))
					* pow(1. - x, double(abs(m - l)) * 0.5)
					* pow(1. + x, double(abs(m + l)) * 0.5);
			dn = d;
			dnm1 = 0.;
			n = 1;
			for (n0 = n0; n0 < N; n0++) {
				if (n0 == 0)
					coefs[row][col].t[n] = d = double(n0 + n0 + 1) * x * dn
							- dnm1;
				else
					d = 1.
							/ (double(n0)
									...
									* sqrt(
											double((n0 + 1) * (n0 + 1))
													- double(l * l)))
							* (double(n0 + n0 + 1)
									* (double(n0 * n0 + n0) * x - double(m * l))
									* dn
									- double(n0 + 1)
											* sqrt(double(n0 * n0 - m * m))
											* sqrt(double(n0 * n0 - l * l))
											* dnm1);
				dnm1 = dn;
...
void WignerFunctions::assign(const int &nn, const double &thet) {
	...
	for (m = -N; m <= N; m++) {
		...
		for (l = -N; l <= N; l++) {
			...
			ee = l < m ? powm1(m - l) : 1.;
			coefs[row][col].t[0] = dn = ee * pow(2., -double(n0))
					* sqrt(
							exp(
									lns[n0 + n0] - lns[int(abs(m - l))]
											- lns[int(abs(m + l))]))
